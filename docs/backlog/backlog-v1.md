...
| **M6ter** — UX par canal (email ≠ WhatsApp) | ⬜ **à faire** — décidé le **2026-07-20** après test en production de M6bis | Cesse de forcer une UX unique sur deux canaux qui n'ont ni la même forme de message ni le même système d'objet. **Ancre `null` pour l'email** + ⚠️ **balayage du fil ENTIER à l'ouverture** (amont compris — la règle actuelle « ≥ ancre » ne rattacherait qu'**un** email sur un fil de six) ; **rendu email pleine largeur sans fond coloré** (bulles conservées en WhatsApp) ; **gestes par canal** (« Supprimer »/rouge vs « Ignorer »/orange — ⚠️ **même `ignoreConversation`, aucune donnée supprimée**) ; **tap message = WhatsApp uniquement** ; **défaut d'ancre = le DERNIER message, toujours** (règle savante « plus ancien non couvert » **supprimée** le 2026-07-20 : imprévisible) corrigé par une **poignée d'ancre glissable** dans un cordon épaissi (**dnd-kit déjà présent**, ⚠️ aperçu en direct obligatoire pendant le drag) ; **bandeau « Suivi dans »** en en-tête de conversation email **au lieu du cordon** (l'ancre nulle rend tout signal par message identique, donc muet) ; **groupes WhatsApp ancrés comme les directs**. **Aucune migration** (`anchor_message_id` déjà nullable). Fond de la décision : **l'ancre est la prothèse d'un objet manquant** — elle tombera avec M7. Détail : §M6ter. |
...
- [ ] **M6ter.9** — Tests : ouverture email → ancre `null` **et fil entier rattaché** (y compris messages antérieurs) ; ouverture WhatsApp **direct et groupe** → ancre posée sur le **dernier message**, messages ≥ ancre ; swipe gauche email → `Conversation.status = ignoré` et **aucune suppression** ; déplacement d'ancre dans les deux sens (entrée et sortie de messages) ; un message **explicitement rattaché à un autre sujet** n'est pas repris par un déplacement d'ancre.
