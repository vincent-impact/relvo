...
| **M6ter** — UX par canal (email ≠ WhatsApp) | ⬜ **à faire** — décidé le **2026-07-20**, **corrigé et SIMPLIFIÉ le 2026-07-21** | **Vocabulaire : un sujet ÉCOUTE une conversation** (« fenêtre » est retiré ; `anchor_message_id` / `closing_message_id` **sont** le début et la fin d'une écoute — aucune migration de plus). Côté **email** un sujet n'écoute rien : il **EST** le fil (⚠️ **balayage du fil ENTIER**, amont compris). ⚠️ **SUPPRIMÉS le 2026-07-21** : le **cordon** (rail coloré par message), la **poignée d'ancre** glissante, dnd-kit sur l'ancre, l'aperçu en direct, le **défaut d'ancre**, le **tap-pop-up** sur message, et le **rattachement/détachement message par message**. **Gestes** : email = swipe droite sur la **CONVERSATION** (nouveau sujet / rattacher) ; WhatsApp = swipe droite sur le **MESSAGE** (crée le sujet **et** remonte une écoute existante), **aucun swipe droite sur une ligne de conversation WhatsApp** ; **tap = pièces jointes uniquement**. **Bandeau « Suivi dans » + « N sujets passés » sur les DEUX canaux.** ⚠️ **Règle de réception CANAL-DÉPENDANTE** (rétablie le 2026-07-21) : **email** = le message rejoint **toujours** le sujet du fil et le **ROUVRE** s'il était terminé, seule l'**ignorance de la conversation** le fait taire ; **WhatsApp** = le message n'alimente le sujet que si l'**écoute est active**. **Arrêts d'écoute (WhatsApp)** : `fermé` → toutes cessent ; `validé` → la conversation n'alimente plus ; **ignorer = PAUSE** (2 canaux) avec **confirmation nommant les sujets**. **« Fermer » = suppression douce** → onglet **Fermés** + **Remettre** (⚠️ **ne redémarre pas les écoutes** ; **aucune purge, jamais**). **Fiche sujet : une seule conversation affichée**, ligne sélecteur → feuille de gestion des écoutes. ⚠️ **Le 1:1 email est DIRECTIONNEL** : conversation → sujet = **1:1**, sujet → conversations = **1:N** — d'où « **rattacher à un sujet existant** » au swipe droite email, seul dispositif pour le **changement d'objet** (Cas M) **et le changement d'adresse** d'un interlocuteur (Cas X, 2026-07-21). Détail : §M6ter. |
...
| Réouverture auto d'un sujet `validé` / `fermé` à réception | **CONSERVÉE côté EMAIL** (un fil email *est* le sujet — cf. M6ter.6quater) ; **supprimée côté WhatsApp**, où une écoute arrêtée ne reprend rien |
...
| ~~**réouverture automatique** d'un sujet email à la réception → « SUPPRIMÉE »~~ | ⚠️ **ligne CADUQUE — la règle est RÉTABLIE** (même jour, cf. M6ter.6quater). Elle avait été retirée sur une lecture erronée de « `validé` n'est plus alimenté », phrase écrite dans une section « **arrêt des écoutes** », donc portant sur **WhatsApp seul** — un sujet email n'écoute rien, il **EST** le fil |
...
- [ ] **M6ter.5bis** — UI : ⚠️ **« Rattacher à un sujet existant » est OBLIGATOIRE au swipe droite email, à parité avec « Ouvrir un sujet »** — pas une option secondaire, pas un écran caché. C'est le **seul** dispositif prévu pour deux situations courantes : (1) le fournisseur ouvre un **nouvel objet** sur une affaire déjà suivie (Cas M) ; (2) l'interlocuteur répond depuis une **autre adresse** — alias, changement d'employeur, envoi depuis un mobile — ce qui produit une **clé différente**, donc une **conversation distincte** (Cas X, décision du 2026-07-21). **Aucun rapprochement automatique n'est tenté à la réception** : détecter « même objet, autre adresse » demanderait de l'**inférence**, ce qui détruirait le **déterminisme** de la clé et la **stabilité de l'identité** d'une conversation. L'humain tranche, en un geste qu'il connaît déjà. Le sélecteur de sujet propose les sujets **ouverts**, les plus récemment actifs en tête. ⚠️ **Prérequis de lecture** : « un fil d'email EST un sujet » est **DIRECTIONNELLE** — conversation → sujet = **1:1**, sujet → conversations = **1:N**. Implémenter le 1:1 en bidirectionnel (contrainte d'unicité sur `subject_id`, ou refus d'une seconde ligne `SubjectConversation`) **casserait ce geste, et avec lui les cas M, S et X**.
...
- [ ] **M6ter.6ter** — Domaine : **arrêt des ÉCOUTES — donc WhatsApp**. `fermé` → **toutes** les écoutes du sujet s'arrêtent (`closing_message_id` posé sur chaque conversation **WhatsApp**), qui **ne référencent plus** ce sujet. `validé` → la conversation **WhatsApp** n'alimente plus le sujet. ⚠️ **Les conversations EMAIL du sujet ne sont PAS touchées** (cf. M6ter.6quater). **Ignorer une conversation = PAUSE** (les deux canaux) : elle n'alimente plus **aucun** sujet ouvert, mais **aucune borne de fin n'est posée** — réactiver fait **reprendre** (sans quoi « Réactiver » serait un bouton sans effet).
- [ ] **M6ter.6quater** — Domaine : ⚠️ **RÉOUVERTURE AUTOMATIQUE D'UN SUJET EMAIL À LA RÉCEPTION — la règle est RÉTABLIE (2026-07-21).** Un nouvel email de même objet + même interlocuteur **rejoint TOUJOURS le sujet du fil, quel que soit son statut**, et le fait passer de `validé` / `fermé` à **`ouvert`** (`closed_at` effacé). ⚠️ **Elle ne contredit PAS « un sujet validé n'est plus alimenté »** : cette phrase est écrite dans une section « **arrêt des écoutes** » et ne vise donc que les **écoutes**, c'est-à-dire **WhatsApp** — un sujet email **n'écoute rien**, il **EST** le fil. **Enjeu concret** : un fournisseur relance sur une affaire validée il y a trois jours ; soit son message rouvre le sujet et remonte dans le fil, soit il s'y range en silence et l'utilisateur **rate exactement le message qu'il ne fallait pas rater**. **Le seul geste qui fait taire un fil email est `ignoreConversation`** — c'est pour cela qu'il est enchaîné à « Valider » et « Fermer ». Côté **WhatsApp**, aucune réouverture : un message arrivant après l'arrêt d'une écoute reste sans sujet, la conversation redevient orpheline et repasse au KPI « Sans sujet ». Détail : `03-cas-usage.md` **Cas W**.
...
- [ ] **M6ter.9** — Tests : ouverture email → ancre `null` **et fil entier rattaché** (y compris messages antérieurs) ; ouverture WhatsApp **direct et groupe** par swipe sur un message → ancre = ce message, messages ≥ ancre ; **swipe sur un message antérieur alors qu'une écoute existe → l'écoute remonte** et les messages traversés reçoivent le `subject_id` ; **valider / fermer → `closing_message_id` posé sur chaque conversation WhatsApp écoutée, et sur AUCUNE conversation email** ; ⚠️ **nouvel EMAIL après un statut terminal → le message rejoint le sujet ET le sujet repasse en `ouvert`** (`closed_at` effacé) ; ⚠️ **nouveau message WHATSAPP après un statut terminal → PAS de réouverture**, message sans sujet, conversation orpheline ; **conversation email `ignoré` → aucun rattachement, aucune réouverture** (c'est le seul moyen de faire taire un fil email) ; **ignorer puis réactiver → l'alimentation reprend** et `closing_message_id` est resté `null` ; **ignorer une conversation écoutée → confirmation NOMMANT les sujets** ; **fermer puis Remettre → sujet `ouvert`, tâches et journal intacts, écoutes WhatsApp NON redémarrées** ; **aucune purge d'un sujet `fermé`, quelle que soit son ancienneté** ; swipe gauche email → `Conversation.status = ignoré` et **aucune suppression** ; ⚠️ **un sujet accepte PLUSIEURS conversations** — rattacher un second fil email à un sujet qui en porte déjà un **réussit** (aucune contrainte d'unicité sur `subject_id` dans `SubjectConversation`), le fil entier du second est balayé, et les deux fils alimentent ensuite le sujet **indépendamment** (cf. Cas M / Cas X).
...
| ~~**Réouverture automatique** d'un sujet email à la réception~~ | ⚠️ **RIEN N'EST CADUC ICI — la règle est MAINTENUE** (rétablie le 2026-07-21, cf. M6ter.6quater et `03-cas-usage.md` Cas W). Ce qui est caduc, c'est la ligne qui la déclarait supprimée |
...
- **M9.18 ✅ — Réalignement docs ⇄ code (point 0) [PRIORITAIRE]** — conception/spec réalignés avec le code (commit `f3f3eda`). Dérives traitées :
  > ⚠️ **Compte rendu d'époque (2026-06), conservé tel quel.** Les valeurs de statut citées ci-dessous (`acknowledged`/`resolved`/`archived`/`ignored`, ignorance « collante », messages orphelins avec rétention 15 j) sont **caduques** depuis M6bis/M6ter : le statut compte 3 valeurs (`ouvert`/`validé`/`fermé`), l'ignorance a migré sur la Conversation, et il n'existe plus de message orphelin. Lire §M6bis et §M6ter pour l'état réel.
