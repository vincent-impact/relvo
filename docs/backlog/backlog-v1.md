...
- **M5.13** ✅ — **Rafraîchissement** (`PollRefresh`, anticipe M12.3) : re-fetch serveur toutes les **20 s** (onglet visible + retour d'onglet) sur fiche sujet / Sujets / Messages — contre le **Router Cache client de Next** (`staleTimes.dynamic = 30 s`) et l'**absence de WebSocket** en V1. Les routes tenant restent en `no-store` (ce n'est pas un cache CDN).
