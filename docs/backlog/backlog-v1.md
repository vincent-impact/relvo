...
| **M6ter** — UX par canal (email ≠ WhatsApp) | ⬜ **à faire** — décidé le **2026-07-20**, **corrigé et simplifié le 2026-07-21** | **Énoncé central : un fil d'email EST un sujet ; une conversation WhatsApp est un FLUX, où la fenêtre et son ancre fabriquent l'objet que le médium ne fournit pas.** Côté **email**, il n'y a **AUCUNE notion de fenêtre** : lien conversation ↔ sujet **1:1 et permanent**, ⚠️ **balayage du fil ENTIER** (amont compris), et un nouveau message **ROUVRE** le sujet. Côté **WhatsApp**, fenêtre à **deux bornes** : **nouvelle colonne `closing_message_id`** (nullable, aucun backfill) — **ignorer = PAUSE** (aucune ancre de fin, réactiver fait reprendre), **valider/fermer = FIN** (le cordon se referme). ⚠️ **`Subject.closed_at` est DÉCLASSÉ** en simple date : appartenance ≠ statut. **Une seule primitive de domaine**, à **ancre optionnelle**, testée sur **l'ancre jamais sur le canal** ; on ouvre **toujours depuis une conversation**. Reste inchangé : **rendu email pleine largeur sans fond coloré** ; **gestes par canal** (« Supprimer »/rouge vs « Ignorer »/orange — **même `ignoreConversation`, aucune donnée supprimée**) ; **défaut d'ancre = le DERNIER message, toujours**, corrigé par la **poignée glissable** (dnd-kit, aperçu en direct obligatoire) ; **bandeau « Suivi dans »** en en-tête email au lieu du cordon ; **groupes ancrés comme les directs**. Détail : §M6ter. |
...
⚠️ **Le `groupe` ne fait PAS exception** (précision du 2026-07-20) : le **nom du groupe ne joue pas le rôle d'un objet d'email**. Il nomme un collectif, pas une affaire — un groupe s'ancre **exactement** comme un direct.

> **L'ancre n'a jamais été un concept du modèle : c'est la PROTHÈSE d'un objet manquant.** Là où l'objet existe (email), **il n'y a aucune notion de fenêtre** ; là où il manque (WhatsApp), elle est indispensable. Quand M7 saura découper un flux **par le sens**, cette prothèse tombera — exactement comme la « fenêtre active », déjà documentée comme un échafaudage (`01-principes.md §9`).

#### 🔑 Correction et simplification du 2026-07-21

> **Un fil d'email EST un sujet. Une conversation WhatsApp est un FLUX ; la fenêtre et son ancre y FABRIQUENT l'objet que le médium ne fournit pas.**

Ce qui change par rapport à la rédaction du 2026-07-20 :

| Écrit le 2026-07-20 | Devenir |
|---|---|
| « côté email, l'ancre est nulle » (mais fenêtre quand même) | **il n'y a AUCUNE notion de fenêtre côté email** — lien conversation ↔ sujet **1:1 et permanent**, appartenance **totale et définitive** |
| « tout le fil **non encore couvert** » | **caduc** — jamais de second sujet sur un fil email : c'est **tout le fil**, littéralement |
| `closed_at` = « borne haute de la fenêtre » | **déclassé** — simple date de clôture, ne gouverne **plus rien** de l'appartenance |
| « pas de réouverture d'un sujet clos » | **faux côté email** — un nouveau message rejoint le sujet **et le rouvre** |
| deux primitives d'ouverture (par message / par conversation) | **une seule**, à **ancre optionnelle** ; on ouvre **toujours depuis une conversation** |
| ignorer / valider ≈ même effet | **PAUSE vs FIN** — l'ignorance ne pose **aucune** ancre de fin |

**Dépendances** : M6bis. **Migration** : une **colonne nullable** `SubjectConversation.closing_message_id`, **aucun backfill** (`anchor_message_id` était déjà nullable).

- [ ] **M6ter.0** — Migration : **`SubjectConversation.closing_message_id: UUID nullable`**, symétrique d'`anchor_message_id`. `null` = fenêtre encore ouverte ; posée = le cordon s'est refermé sur ce message. ⚠️ **Aucun backfill** — les lignes existantes valent « encore ouverte », ce qui est exactement leur état. **Pourquoi une borne qui désigne un message** plutôt qu'une borne déduite de `closed_at` : (1) `closed_at` est déclassé en simple date ; (2) une borne calculée sur un **horodatage** devient fausse dès que **deux messages arrivent dans la même seconde** — **une borne qui désigne un message ne ment jamais**.
- [ ] **M6ter.1** — Domaine : **une seule primitive d'ouverture, à ancre OPTIONNELLE** — *ouvrir un sujet **sur une conversation**, `anchorMessageId?`*. Ancre `null` → **tout le fil** ; ancre posée → **à partir d'elle**. ⚠️ **La fonction teste l'ANCRE, jamais le canal** — un `if (channel === 'email')` dans le domaine est le premier pas vers deux produits (c'est la garde déjà écrite : *le canal décide du geste, jamais de la fonction appelée*). C'est l'UI qui décide quelle valeur d'ancre transmettre : `null` en email, le message désigné (ou le dernier reçu) en WhatsApp.
...
- [ ] **M6ter.3** — UI : **rendu email pleine largeur** dans le détail de conversation — emails enchaînés au fil du scroll (reprise de l'ancien `/messages/[id]`), **fond blanc dans les deux sens**, en-tête porteur (avatar + expéditeur + date), sortant signalé par « **Moi** ». **WhatsApp conserve les bulles.** ⚠️ **Pas de fond teinté sur l'email** : sur du texte long il fatigue et abîme la lisibilité — c'est précisément ce qu'on vient gagner en sortant de la bulle (cf. Gmail / Superhuman / Outlook). Repli si insuffisant à l'usage : teinte **très légère au sortant seulement**.
...
- [ ] **M6ter.6** — UI : **tap sur message = WhatsApp uniquement**. Retrait de la pop-up message côté email. ⚠️ **Le point d'entrée d'ouverture est TOUJOURS la conversation** (2026-07-21), sur les deux canaux : côté WhatsApp le tap **désigne l'ancre** à transmettre à la primitive unique (M6ter.1), il n'ouvre pas un second chemin.
- [ ] **M6ter.6bis** — Domaine : **réouverture d'un sujet email à la réception**. Un nouvel email de même objet + même interlocuteur rejoint **toujours** son sujet (lien 1:1 permanent) — **même si le sujet est `validé` ou `fermé`** — et le fait **repasser en `ouvert`** (`closed_at` effacé, `EventLog` « sujet rouvert », actor `system`). Justification : de l'activité sur une affaire signifie qu'elle est **vivante**, et l'appartenance ne dépend pas du statut. ⚠️ **Le seul geste qui fait taire un fil est d'ignorer la conversation** — un mécanisme d'extinction, pas deux qui se ressemblent. **Sans objet côté WhatsApp** (la fenêtre y est refermée par `closing_message_id`). Cas U.
- [ ] **M6ter.6ter** — Domaine : **PAUSE vs FIN**. `ignoreConversation` **ne pose AUCUNE ancre de fin** (`closing_message_id` reste `null`) : le sujet cesse d'être alimenté, mais **réactiver la conversation fait REPRENDRE l'alimentation**. À l'inverse, **valider ou fermer un sujet WhatsApp pose `closing_message_id`** sur le dernier message reçu — le cordon se referme, la conversation redevient orpheline. ⚠️ Sans cette distinction, « Réactiver » (cas R) serait un bouton **sans effet observable**.
- [ ] **M6ter.7** — Domaine : **défaut d'ancre au swipe droite (WhatsApp, directs ET groupes) = le DERNIER message. Toujours.** Aucun cas, aucune exception, aucune borne temporelle, aucune dépendance à l'historique de rattachement du fil. ⚠️ **Supprime** la règle « plus ancien message non couvert, borné par la fenêtre précédente » et son exception « conversation vierge » : elle tombait juste plus souvent, mais restait **imprévisible** — il fallait reconstituer de tête les sujets passés du fil pour anticiper son propre geste. **Un défaut simple et prévisible, corrigé à la main quand il se trompe, vaut mieux qu'une règle savante que personne ne peut anticiper** ; la correction, c'est M6ter.8. Pas de sélecteur dans le parcours du swipe (redondant avec le tap message, tue sa vitesse, décision au mauvais moment).
- [ ] **M6ter.8** — UI + domaine : **poignée d'ancre — le cordon WhatsApp est ÉPAISSI** pour que son **nœud de départ** devienne une **poignée saisissable**, que l'on **attrape et fait glisser** vers le haut / le bas dans le cordon : remonter fait **entrer** les messages antérieurs, descendre les fait **sortir** (`subject_id = null`). C'est le mécanisme de correction du défaut M6ter.7 (cas T). **Sans objet côté email.** ⚠️ **Réutiliser `dnd-kit`, déjà dans le projet** (semainier Accueil, planning mois) — pas de seconde librairie de drag ; réglages de départ éprouvés : collision `pointerWithin`, `DragOverlay` centré sur le curseur.
- [ ] **M6ter.8bis** — UI : ⚠️ **aperçu en direct PENDANT le drag de la poignée** — surbrillance des messages qui **entrent**, estompage de ceux qui **sortent**, cordon qui s'étire/se rétracte en suivant le doigt. **Exigence, pas raffinement** : déplacer l'ancre **réécrit des `Message.subject_id`** ; sans aperçu, l'utilisateur relâche à l'aveugle et découvre après coup ce qu'il a décidé — c'est réintroduire par la porte de derrière l'imprévisibilité qu'on vient de retirer en M6ter.7.
- [ ] **M6ter.8ter** — UI : **bandeau « Suivi dans » en en-tête de conversation EMAIL** — pastille de couleur du **domaine** (`Folder` du sujet) + **titre du sujet**, **cliquable vers la fiche du sujet**. ⚠️ **Et AUCUN cordon / rail de couleur par message côté email** : l'ancre étant nulle, tout le fil appartient au sujet, donc un signal par message serait **identique partout** — porteur d'aucune information. L'appartenance ne varie qu'au niveau de la **conversation** : c'est là que le signal se pose. Le cordon reste entier de sens en WhatsApp, où l'appartenance **varie d'un message à l'autre**.
- [ ] **M6ter.9** — Tests : ouverture email → ancre `null` **et fil entier rattaché** (y compris messages antérieurs) ; ouverture WhatsApp **direct et groupe** → ancre posée sur le **dernier message**, messages ≥ ancre ; **email validé + nouveau message → message rattaché ET sujet repassé en `ouvert`** (M6ter.6bis) ; **WhatsApp validé + nouveau message → conversation orpheline, sujet inchangé** ; **ignorer puis réactiver → l'alimentation reprend** et `closing_message_id` est resté `null` (M6ter.6ter) ; swipe gauche email → `Conversation.status = ignoré` et **aucune suppression** ; déplacement d'ancre dans les deux sens (entrée et sortie de messages) ; un message **explicitement rattaché à un autre sujet** n'est pas repris par un déplacement d'ancre.

#### Écarté de la V1 — plusieurs sujets SIMULTANÉS sur une même conversation

Hypothèse **mise de côté**, et son coût d'ajournement est **nul** : `SubjectConversation` est **déjà** une table de liaison **plusieurs-à-plusieurs**, donc le schéma l'autorise depuis le premier jour. Ce qui l'interdit n'est qu'une **règle métier V1** (« au plus un sujet ouvert par conversation »), levable **sans migration** — c'est déjà documenté ainsi. Le jour où on la lève : **uniquement du rendu** (aligner les cordons horizontalement), **aucune donnée à reprendre**.

Le cas jugé **plus probable** — des sujets **successifs** sur un même fil, sans chevauchement — **fonctionne déjà** avec la paire début/fin : ce sont des **fenêtres disjointes** sur le même flux.

#### ⚠️ Ce que ce module rend caduc

| Élément | Devenir |
|---|---|
| « Un sujet s'ouvre **toujours** depuis un message d'ancrage » | **caduc** — on ouvre **toujours depuis une conversation** ; le tap WhatsApp ne fait que **désigner l'ancre** (2026-07-21) |
| Deux primitives d'ouverture (par message / par conversation) | **une seule**, à **ancre optionnelle** (2026-07-21) |
| Balayage « messages ≥ ancre » **appliqué à l'email** | **caduc** — l'email balaie le fil **entier**, amont compris |
| « le sujet email couvre tout le fil **non encore couvert** » | **caduc (2026-07-21)** — jamais de second sujet sur un fil email : c'est **tout le fil** |
| `Subject.closed_at` = « **borne haute de la fenêtre** » | **déclassé (2026-07-21)** — simple date, ne gouverne plus l'appartenance |
| « Il n'y a **pas de réouverture** d'un sujet clos » | **faux côté email (2026-07-21)** — un nouveau message rouvre le sujet ; reste vrai en WhatsApp |
...
| « Chaque conversation d'un sujet a **sa propre ancre** » | reformulé : chaque conversation a le **régime d'ancre de son canal**, et **deux** ancres possibles (début / fin) |
