...
| #        | Item                                                                                                                                                                                                                                                                       |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M15.1    | **Frontmatter** sur les épiques : `id`, `public`, `ordre_public`, `titre_client`, `resume_client`, `statut`, `debut`, `fin`. ✅ _posé_                                                                                                                                     |
...
| M15.8    | **Definition of Done** — la ligne `Client:` et la mise à jour du frontmatter à chaque changement d'état. ✅ _posé_                                                                                                                                                         |
...
- [x] Le corps d'un fichier d'épique **n'apparaît nulle part** dans le JSON généré.
      _(Le vérifier en écrivant une phrase absurde dans un corps et en relisant le JSON.)_
- [x] Un commit `chore:` **n'apparaît pas** dans le journal ; un commit `feat:` avec
      ...
      _(Vérifié sur les 148 commits réels : 143 sans pied → 0 publié.)_
...
- [x] Le test du refus par défaut passe, et **il échoue** si l'on retire temporairement une route
      de la liste publique. _(C'est la seule façon de savoir qu'il teste quelque chose.)_
...
> _Le traceur analyse les `import`/`require`/`fs` **statiquement** : il ne peut pas voir ce chemin
> construit à l'exécution, donc il n'embarque pas les fichiers. En local ça marche (le monorepo
> est sur le disque) ; en production, `readFile` lève `ENOENT`._
