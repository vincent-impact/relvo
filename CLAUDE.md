...
11. ⚠️ **« Conversations par contact, tous canaux confondus » est CADUC** (2026-07-20). La `Conversation` est une **entité** avec **deux sous-types** (refonte 2026-07-25) : **E-MAIL** (a un objet + un **set de destinataires**, clé `email:<objet>:<set>`, rattachement **permanent** au sujet) et **MESSAGERIE** (WhatsApp — demain Instagram/Messenger/LinkedIn : un contact **ou** un groupe, clés `wa-direct:<numéro>` / `wa-group:<chat_id>`, que le sujet **écoute**). Un même contact qui écrit par e-mail puis par messagerie produit **deux conversations** ; la réunification **remonte au Sujet**, qui agrège 0/1/n conversations. ⚠️ **Fiche sujet — DEUX onglets par canal** (remplace « une seule conversation à la fois » + ligne-sélecteur + feuille de gestion, décision du 2026-07-21 **abandonnée**) : onglet **E-mail** (enveloppe ; conversations **par destinataire**, « Groupe » si ≥ 2 ; « Ajouter » = *écrire un e-mail*, objet pré-rempli = titre du sujet ; `unlink` = **détacher**, rattrapage d'erreur) et onglet **Messagerie** (double bulle ; conversations **écoutées** ; « Ajouter » = *choisir un fil existant*, écran vierge, écoute dès maintenant ; `unlink` = **arrêter l'écoute**). Le **composer est propre à chaque onglet**. La garde « pas d'onglets dans des onglets » est **levée en connaissance de cause**. Chips vs sous-onglets par destinataire = **à valider à l'usage**. Le terme **« Interlocuteur(s) »** remplace « Destinataire(s) » dans l'UI.
...
13bis. **UX par canal — on diverge sur le RENDU, les GESTES et la SURFACE (2 sous-types + 2 onglets), jamais sur le DOMAINE** (2026-07-20, simplifié le 2026-07-21, **refonte du 2026-07-25**). Forcer une UX unique desservait les deux canaux et produisait des patterns complexes qui égaraient l'utilisateur : les messages e-mail sont longs et structurés (la bulle les étrangle) et ont un **objet**, la messagerie non. On **sépare** désormais en deux sous-types de conversation et deux onglets (cf. n°11).
