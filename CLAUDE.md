...
| `/dossiers/[id]` · `/dossiers/nouveau` | **Domaines** (ex-« Mémoire ») — fiche d'un domaine : onglets Instructions/Documents/Sujets. ⚠️ **La LISTE des domaines a quitté la nav le 2026-07-28** : elle vit dans l'onglet **Domaines** des Réglages (sans barre KPI). L'index `/dossiers` **redirige** vers `/parametres?tab=domaines` | Hors-nav (via l'onglet Domaines des Réglages) |
| `/parametres` | **Réglages** (⚙️) — onglets Profil / Canaux / **Domaines** (liste des domaines, ex-« Mémoire ») / Préférences | Onglet |
| `/contacts` · `/contacts/[id]` | **Contacts** (annuaire) + fiche contact | Onglet (**Contacts**, a remplacé Mémoire dans la nav le 2026-07-28) |
| `/sujets/[id]` | Détail d'un sujet | Fiche détail |
| `/planning` | Calendrier vue mois | Hors-nav (lien depuis le widget semaine de l'Accueil) |
| `/conversations` · `/conversations/[id]` | **Conversations** (💬 bulles, ex-« Messages »/✉️ jusqu'au 2026-07-28 ; même icône que l'onglet Conversations d'un sujet) — liste triée par activité **PUREMENT temporelle** (le non-lu ne remonte plus, 2026-07-23), filtres **Sans sujet** (défaut) / Ignorées / Toutes + filtre canal. **Détail (v2, 2026-07-23)** : tout le contexte (interlocuteur, canal en clair « E-mail »/« WhatsApp », domaine, résumé placeholder, sujets suivis) vit DANS le hero violet ; l'avatar de l'expéditeur est cliquable (contact connu → sa fiche · « ? » → création pré-remplie) ; le dock 5-icônes cède la place à un **dock d'action violet** — « Ignorer » (fantôme) / « Ouvrir un sujet » (blanc plein). ⚠️ **Rendu et gestes par canal** (cf. invariant n°13bis) | Onglet (**Conversations**, ex-« Messages », entre Sujets et Contacts) |

**Navigation = barre d'onglets en bas** (mobile-first), **5 entrées** (décision 2026-07-23, ex-4) : **Actions** 🗓️, **Sujets** 📦, **Conversations** 💬 (ex-« Messages » jusqu'au 2026-07-28), **Contacts** (annuaire, ex-🧠 Mémoire depuis le 2026-07-28), **Réglages** ⚙️. **« Conversations » (→ `/conversations`) a rejoint le dock**, entre Sujets et Contacts : les conversations comptent tant qu'aucune IA ne fait le tri, et la barre rend visible la chaîne de transformation **Actions ← Sujets ← Conversations**. Sur le **détail** d'une conversation (`/conversations/[id]`), le dock est masqué au profit du dock d'action (« Ignorer » / « Ouvrir un sujet »). « Accueil » est devenu **Actions** (page des tâches, cf. invariant n°30) et « Mon fil » est devenu **Sujets**. ⚠️ **Permutation Contacts ↔ Mémoire + renommages d'icônes (2026-07-28)** : **Contacts** (annuaire `/contacts`, icône **deux personnes/Users**) a **repris la place de « Mémoire »** dans la nav ; en échange la **liste des domaines** (ex-page « Mémoire ») est devenue l'**onglet « Domaines » des Réglages** (sans barre KPI, liste seule) et **« Mémoire » a disparu de la nav**. Le même jour, **« Messages » a été renommé « Conversations »** avec l'**icône bulles** (`MessagesSquare`, la même que l'onglet Conversations d'un sujet). Icônes actuelles : Actions = **agenda**, Sujets = **carton/projet**, Conversations = **bulles**, Contacts = **deux personnes**, Réglages = **engrenage**. Note IA : un contact rattaché à un **pôle/dossier** influencera la qualification des messages reçus (les sujets d'un contact « RH » tomberont probablement dans le dossier RH). L'**échange** avec Relvo (`/relvo`) est une **surface plein écran** (plus un drawer latéral), atteinte via un **bouton Relvo en haut à droite du header violet** — présent sur toutes les pages, même forme que l'ancien bouton ✦ du composer (décision 2026-06-27, abandon du composer persistant du bas : trop encombrant, hidden-menu, confusion avec le composer destinataire d'un sujet). Les éventuels boutons de page (ex. « + » Nouveau sujet) se posent **à gauche** du bouton Relvo. La **barre d'onglets basse est fixe, sur fond violet** (comme l'ancien composer). Le virage mobile-first et la PWA sont détaillés dans `docs/spec/ux-mobile-first.md` ; cf. `01-principes.md §13`.
...
11. ⚠️ **« Conversations par contact, tous canaux confondus » est CADUC** (2026-07-20). La `Conversation` est une **entité** avec **deux sous-types** (refonte 2026-07-25) : **E-MAIL** (a un objet + un **set de destinataires**, clé `email:<objet>:<set>`, rattachement **permanent** au sujet) et **MESSAGERIE** (WhatsApp — demain Instagram/Messenger/LinkedIn : un contact **ou** un groupe, clés `wa-direct:<numéro>` / `wa-group:<chat_id>`, que le sujet **écoute**). Un même contact qui écrit par e-mail puis par messagerie produit **deux conversations** ; la réunification **remonte au Sujet**, qui agrège 0/1/n conversations. ⚠️ **Fiche sujet — une LISTE de conversations, pas un fil embarqué** (2026-07-27, **remplace les « DEUX onglets par canal » du 2026-07-25**, jugés flottants/dupliqués à l'usage). La fiche n'affiche plus les messages : un **onglet unique « Conversations »** liste les fils du sujet (icône de canal, **objet**/nom, **extrait du dernier message**, non-lus). Cliquer une ligne ouvre la conversation dans **`/conversations/[id]`**, désormais **seule surface d'affichage ET de réponse** (on gagne un onglet et on cesse de dupliquer le rendu d'un fil). ⚠️ **Répondre se fait DANS l'écran conversation** : quand un fil est **rattaché** à un sujet ouvert, son **dock du bas devient le composer** (e-mail = reply-all sur le set ; messagerie = fil `chat_id`) ; **détacher** (e-mail) / **arrêter l'écoute** (messagerie) vivent dans le bandeau **« Suivi dans »** du hero (icône chaîne brisée, geste **par canal**). Un fil **orphelin** garde le dock de triage (Ignorer / Lier / Nouveau). « Ajouter une conversation » reste un bouton de la liste (dialog par canal). Le terme **« Interlocuteur(s) »** remplace « Destinataire(s) » dans l'UI.
12. **Création automatique d'un contact** (pipeline IA) uniquement à la création d'un sujet — l'IA ne crée jamais un contact « dans le vide ». Expéditeur inconnu = `sender_raw` + avatar `?`. **La création manuelle par l'utilisateur reste permise** (bouton + de l'annuaire `/contacts` → `sourceActor: user`, statut `complete` d'emblée). ⚠️ **Fiche contact « classique » (2026-07-28)** : Prénom · Nom · **Entreprise** · **Téléphone(s)** · **Email(s)**, avec **plusieurs numéros et adresses** (primaire = `email`/`phone` = « Téléphone 1 »/« Email 1 » ; secondaires = `additional_emails[]` / `additional_phones[]`, migration additive). La **« Fonction » (`job_title`) est retirée de l'UI** (colonne conservée en base, plus éditable). ✅ **L'auto-rattachement des messages entrants consulte le primaire ET les secondaires** (2026-07-28) : un e-mail/WhatsApp venu d'une adresse ou d'un numéro secondaire résout le bon contact. Les e-mails secondaires sont **stockés en minuscules** (le match tableau `has`/`hasSome` est exact — pas de `mode: insensitive` sur les tableaux Postgres) ; helpers `contactMatchesEmail` / `contactMatchesPhone` / `contactMatchesAnyEmail` dans `contacts.ts`.
13. Sujets **multi-contacts** (`Subject.contact_ids: UUID[]`). ⚠️ Depuis la refonte 2026-07-25, une **conversation E-MAIL** porte elle aussi un **set de contacts** (`Conversation.contact_ids: UUID[]`, 1 = direct, ≥ 2 = « Groupe ») ; une conversation **MESSAGERIE** garde **un seul** `contact_id` (ou groupe = `null`).
13bis. **UX par canal — on diverge sur le RENDU et les GESTES, jamais sur le DOMAINE** (2026-07-20, simplifié le 2026-07-21, refonte 2026-07-25, **onglets fondus en une liste le 2026-07-27**). Forcer une UX unique desservait les deux canaux : les messages e-mail sont longs et structurés (la bulle les étrangle) et ont un **objet**, la messagerie non. On garde **deux sous-types de conversation** (e-mail / messagerie), mais le **rendu par canal vit dans l'écran conversation `/conversations/[id]`** (seule surface) — la fiche sujet n'en montre qu'une **liste** (cf. n°11).
   - **🔑 Énoncé central** : **un fil d'e-mail appartient à un sujet (un seul, à vie) ; une conversation de messagerie est un FLUX, qu'un sujet ÉCOUTE — à partir d'un message, jusqu'à ce qu'il cesse d'écouter.** Tout le reste en découle.
   ...
   - **Réponse d'un destinataire / changement d'adresse — frontière IN-SET / HORS-SET** (décision du 2026-07-25) : e-mail entrant de même objet nominal → si l'expéditeur est **déjà dans le set** du sujet (ex. Sophie répond « à nous seuls » sur `[Karim, Sophie]`), Relvo crée une **nouvelle conversation** (set différent = clé différente) et la **range automatiquement** dans le sujet ; si l'adresse est **totalement nouvelle** (`karim@sogood.fr` jamais vue), la conversation tombe **orpheline** et l'utilisateur la **rattache à la main** (« Rattacher à un sujet existant », Cas M/X). ⚠️ **Le rattachement in-set n'est PAS de l'inférence** : objet normalisé = fonction pure, appartenance au set = test exact — le **déterminisme** tient. Seule reste interdite la devinette « même objet, **adresse inconnue** » (hors-set = manuel), qui détruirait déterminisme et **stabilité de l'identité**. Cf. `03-cas-usage.md` **Cas X**.
   - **Vocabulaire : « ÉCOUTE » remplace « fenêtre »** (2026-07-21). « Écoute » décrit une **action du sujet** (il se branche, il se débranche) là où « fenêtre » décrivait une **plage subie** — et ça déplace la commande dans **la fiche du sujet**. ⚠️ **Le modèle ne change pas** : `anchorMessageId` et `closingMessageId` **SONT** le début et la fin d'une écoute. **L'écoute est un concept exclusivement de la messagerie** (« thread sans objet ») : côté e-mail, un sujet **n'écoute rien**, il **EST** le fil.
   ...
   - **Messagerie — une écoute à DEUX bornes.** `anchorMessageId` (début) et `closingMessageId` (fin, colonne nullable, **aucun backfill**). **Directs ET groupes de messagerie, même régime** (le nom du groupe **ne fait pas office d'objet** ; un groupe **e-mail**, lui, est un fil d'affaire rattaché en permanence).
   - **Arrêt des écoutes — donc, structurellement, la messagerie** (rétabli le 2026-07-21) : **`fermé`** → **plus d'écoute du tout**, la conversation ne référence plus ce sujet ; **`validé`** → la conversation **n'alimente plus** le sujet ; **« arrêter l'écoute »** (`unlink` du bandeau « Suivi dans » de l'écran conversation, messagerie) → cette conversation-là seulement ; **conversation ignorée (mute)** → elle n'alimente plus **aucun** des sujets ouverts qui l'écoutent, **sans borne de fin** (PAUSE réversible sur les **deux** canaux — sinon « Réactiver » serait un bouton sans effet). ⚠️ **Ces lignes ne concernent QUE la messagerie** : les conversations **e-mail** d'un sujet ne sont **jamais** touchées par un changement de statut — elles continuent de l'alimenter et le **ROUVRENT** (cf. n°7 et `03-cas-usage.md` Cas W). ⚠️ **Détacher un fil e-mail** est un geste **distinct** — `unlink` (chaîne brisée) du bandeau « Suivi dans » de l'écran conversation, **rattrapage d'erreur**, **sans** borne de fin (supprime la liaison + retire `subject_id` des messages du fil).
...
16. `Folder` (modèle) = **« Domaines »** (⚠️ ex-« Mémoire » ; **sorti de la nav le 2026-07-28** → onglet **Domaines** des Réglages, liste seule sans KPI) ; chaque Folder = **« un domaine de la mémoire de Relvo »**, fiche en **3 onglets : Instructions / Documents / Sujets** (`/dossiers/[id]`). Regroupe Sujets (`Subject.folder_id`) **et** Connaissances (`KnowledgeDocument.folder_id`).
