...
13bis. **UX par canal — on diverge sur le RENDU et les GESTES, jamais sur le DOMAINE** (décision 2026-07-20, après test en prod de M6bis). Forcer une UX unique dessert les deux canaux : les messages email sont longs et structurés (la bulle les étrangle), et WhatsApp n'a pas d'**objet**.
   - **Rendu** : email = **pleine largeur**, emails enchaînés au scroll, **fond blanc dans les deux sens** — c'est l'**en-tête** (avatar + expéditeur + date) qui porte l'information, le sortant se signalant par « **Moi** » + un **rail de couleur discret** à gauche. ⚠️ **Pas de fond teinté** : sur du texte long il fatigue et abîme la lisibilité, ce qu'on vient précisément gagner en sortant de la bulle (cf. Gmail/Superhuman/Outlook). Repli si insuffisant : teinte **très légère au sortant seulement**. **WhatsApp conserve les bulles.**
   - **Gestes** : cf. n°8. **Tap sur un message = WhatsApp uniquement** ; côté email il n'y a plus de tap, et **on ne peut pas ouvrir un sujet depuis un message** — seulement depuis la **conversation**.
   - **Ancre** : `SubjectConversation.anchorMessageId` est **`null` pour l'email** (déjà nullable → aucune migration). ⚠️ **Ouvrir un sujet email balaie le fil ENTIER, amont compris** — la règle « messages ≥ ancre » est un héritage WhatsApp qui, appliquée à l'email, ne rattacherait qu'**un** message sur un fil de six. Côté WhatsApp l'ancre est posée, **visible et déplaçable** depuis le sujet ; au swipe droite elle vaut par défaut le **plus ancien message non couvert**, sauf sur une conversation n'ayant **jamais** porté de sujet → le **dernier** message (sinon on remonterait des mois).
